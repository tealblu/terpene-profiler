...
def create_graph(terp_data: dict, terp_scores: dict):
    ...
    terp_scores_dict = {
        terp: [score for t, score in terp_scores if t == terp] for terp in unique_terps
    }
    ...
    ax4.axis("off")
    raw_data = [
        f"{terp}: {', '.join(map(str, scores))}"
        for terp, scores in terp_scores_dict.items()
    ]
    combined_text = "\n".join(raw_data)
    ax3.text(
        0,
